#!/usr/bin/env bash
# Upload raw.fib_seq from local staging to prod.

set -e

echo "Uploading raw.fib_seq to PROD..."

DUMP_FILE=fib_seq.dump
...
psql "$REMOTE_CONN" -c "DROP TABLE IF EXISTS raw.fib_seq CASCADE;"
