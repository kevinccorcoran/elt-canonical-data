...
-- --------------------------------------------------------------
-- CANONICAL TICKER ALLOWLIST
--
-- Purpose:
--   Define the authoritative set of tickers that are considered
--   valid / in-scope for downstream pipelines.
--
-- Notes:
--   - SPY is injected explicitly (synthetic / remapped index)
--   - Remaining tickers are pulled from raw metadata
--   - Final output is a simple lookup table keyed by ticker
-- --------------------------------------------------------------

-- Explicitly include SPY (not always present in raw sources)
SELECT
    'SPY' AS ticker,
...
-- Include tickers from raw metadata, restricted to an explicit allowlist
SELECT
    ticker,
    ingested_at
FROM {{ source('raw', 'ticker_index_summary') }}
WHERE ticker IN (
    -- Large explicit allowlist of equities, indices, and crypto symbols
    -- (trimmed here for readability; unchanged logic)
    '^GSPC','A','AA','AAL','AAON','AAP','AAPL','AAT','ABBV','ABCB','ABG','ABM','ABNB','ABR',
  'ABSI','ABT','ACA','ACAD','ACGL','ACHC','ACHR','ACI','ACIW','ACLS','ACM','ACN','ACT',
  'ADEA','ADI','ADM','ADMA','ADNT','ADP','ADPT','ADSK','ADT','ADUS','AEE','AEIS',
  ...
  'APTV','AR','ARCB','ARE','ARES','ARI','ARLO','ARM','ARMK','AROC','ARR','ARW','ARWR',
  ...
  'DIOD','DIS','DJT','DKNG','DKS','DLB','DLR','DLTR','DLX','DNB','DNOW','DOC',
  ...
  'GOGO','GOLF','GOOG','GOOGL','GPC','GPI','GPK','GPN','GRBK','GRMN','GS','GSHD',
  ...
  'HLNE','HLT','HLX','HMN','HNI','HOG','HOLX','HOMB','HON','HOOD','HOPE','HP',
  ...
  'NUE','NVDA','NVR','NVRI','NVST','NVT','NWBI','NWE','NWL','NWN','NWS','NWSA',
  ...
  'RGEN','RGLD','RGR','RH','RHI','RHP','RITM','RIVN','RJF','RKLB','RKT','RL',
  ...
  'SW','SWK','SWKS','SWX','SXC','SXI','SXT','SYF','SYK','SYNA','SYY','T',
